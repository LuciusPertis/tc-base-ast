...
namespace ast
{
  ...
  template <typename D> inline constexpr
  Chunk<D>::const_reference Chunk<D>::operator[](size_type pos) const
  {
    return decs_->operator[](pos);
  }
  ...
  template <typename D> inline constexpr
  Chunk<D>::iterator Chunk<D>::erase(const_iterator first, const_iterator last)
  {
