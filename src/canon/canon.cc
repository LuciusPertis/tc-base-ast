...
namespace canon
{
  ...
  tree::rStm Canon::assemble_stm(const tree::rStm& s1, const tree::rStm& s2)
  {
    ...
    if (s1.is_a<tree::Seq>())
      {
        tree::rSeq seq = s1.cast<tree::Seq>();
    ...
    if (s2.is_a<tree::Seq>())
      {
        tree::rSeq seq = s2.cast<tree::Seq>();
        append_and_move(res->children_get(), seq->children_get());
      }
    else
      res->emplace_back(s2);
  ...
  Canon::reorder(const tree::tree_list_type& l)
  {
    ...
    auto e = do_exp(h.cast<tree::Exp>());

    auto [stm, el] = reorder(rest);

    commute_p commutes(trace_p_);
    if (commutes(stm, e->exp_get()))
      {
        stm = assemble_stm(e->stm_get(), stm).cast<tree::Seq>();

        res.emplace_back(e->exp_get());
      }
    else
      {
        temp::Temp t;

        auto tmp = assemble_stm(
          e->stm_get(), new tree::Move(new tree::Temp(t), e->exp_get()));
        stm = assemble_stm(tmp, stm).cast<tree::Seq>();
  ...
  tree::rEseq Canon::do_exp(const tree::rExp& exp)
  {
