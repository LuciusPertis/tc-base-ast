 ...
 ** \brief Implementation of target::ia32::GasMatcher.
 **/
...
{
  GasMatcher::GasMatcher(GasCodegen* cg)
    : cg_(cg)
  {}
  ...
  void GasMatcher::operator()(tree::rCall call)
  {
    TRACE("call: Call");
  ...
  // FIXME: Some code was deleted here (Store all the arguments in frame).
    ...
    // Now all that the arguments are stored, we can safely jump to the
    ...
    EMIT(ASSEMBLY.call_build(func->label_get(), res));

    call->asm_set(MCPU.result_reg());
  }

  void GasMatcher::operator()(tree::rConst cst)
  {
    TRACE("exp: Const");
    EMIT(ASSEMBLY.move_build(cst->value_get(), cst->asm_get()));
  }

  void GasMatcher::operator()(tree::rMem mem)
  {
  ...
  void GasMatcher::operator()(tree::rName name)
  {
    TRACE("exp: Name");
    EMIT(ASSEMBLY.move_build(name->label_get(), name->asm_get()));
  }

  void GasMatcher::operator()(tree::rTemp temp)
  {
    TRACE("temp: Temp");
    temp->asm_set(temp->temp_get());
  }

  void GasMatcher::operator()(tree::rCjump cjump)
  {
  ...
  void GasMatcher::operator()(tree::rJump jump)
  {
    TRACE("stm: Jump");
  ...
  void GasMatcher::operator()(tree::rLabel label)
  {
    TRACE("stm: Label");
    EMIT(ASSEMBLY.label_build(label->label_get()));
  }

  void GasMatcher::move_load(tree::rExp exp, tree::rBinop binop)
  {
    REDUCE(exp);
  ...
  void GasMatcher::move_store(tree::rExp exp, tree::rBinop binop)
  {
    REDUCE(exp);
  ...
  void GasMatcher::operator()(tree::rMove move)
  {
  ...
  void GasMatcher::operator()(tree::rSeq seq)
  {
    TRACE("stm: Seq");
  ...
  void GasMatcher::operator()(tree::rSxp sxp)
  {
    TRACE("stm: Sxp(exp)");
    REDUCE(sxp->exp_get());
  }
